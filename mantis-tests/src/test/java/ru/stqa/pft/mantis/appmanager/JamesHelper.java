...
public class JamesHelper {
  ...
  private void initTelnetSession() {
    ...
    readUntil("Wellcome " + login + ". HELP for a list of commands");
  }
    private String readUntil(String parrent) {
      try {
        char lasChar = parrent.charAt(parrent.length() - 1);
        ...
        while (true) {
          ...
          sb.append(ch);
          if (ch == lasChar) {
            if (sb.toString().endsWith(parrent)) {
              return sb.toString();
            }
          }
          ch = (char) in.read();
